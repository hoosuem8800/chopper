      ...
      <nav className={`${scrolled ? 'bg-white/95 py-2' : 'bg-white/90 py-3'} md:py-3 py-1.5 backdrop-blur-md shadow-sm z-50 sticky top-0 transition-all duration-300`}>
        ...
        <div className="container mx-auto px-4">
            ...
            <div className="flex items-center gap-2 lg:hidden">
              {isAuthenticated && (
                <button 
                  ...
                  className={`relative rounded-full p-2 transition-all duration-300 active:scale-95 touch-manipulation ${
                    unreadCount > 0
              ...
              <button
                ...
                className="ml-1 p-2 rounded-md hover:bg-gray-100 text-gray-600 transition-colors duration-300 active:scale-95 touch-manipulation"
              >
                {isMenuOpen ? (
                  <X className="h-5 w-5" />
                ) : (
          ...
          {isAuthenticated && mobileNotificationDropdownOpen && (
            <div className="fixed inset-x-0 top-[calc(100%-1px)] mt-0 mx-auto w-full max-w-[95vw] sm:max-w-sm bg-white rounded-b-xl sm:rounded-xl shadow-xl border border-gray-200 overflow-hidden z-50 transform-gpu transition-all duration-200 ease-out">
              {/* Notification Header */}
              <div className="bg-gradient-to-r from-blue-500 to-cyan-400 py-2 px-3">
                <div className="flex justify-between items-center">
                  <h4 className="text-white font-medium text-sm flex items-center gap-1.5">
                    ...
                    {unreadCount > 0 && (
                    <span className="text-xs bg-white text-blue-600 px-1.5 py-0.5 rounded-full font-semibold shadow-sm">
                      {unreadCount} new
                      </span>
                    )}
                </div>
                  ...
                  {notifications.length === 0 ? (
                    <div className="py-6 px-3 text-center">
                      <div className="mx-auto w-12 h-12 mb-3 flex items-center justify-center rounded-full bg-gray-100">
                        <Bell className="h-6 w-6 text-gray-400 animate-pulse-slow" />
                      </div>
                      <h3 className="text-gray-600 font-medium text-sm mb-1">No notifications yet</h3>
                      <p className="text-gray-500 text-xs">We'll notify you when something important happens</p>
                    </div>
                  ) : (
                    <>
                      ...
                      {notifications.filter(n => !n.is_read).length === 0 ? (
                        <div className="py-6 px-3 text-center">
                          <div className="mx-auto w-12 h-12 mb-3 flex items-center justify-center rounded-full bg-green-50">
                            <CheckCircle className="h-6 w-6 text-green-400" />
                          </div>
                          <h3 className="text-gray-600 font-medium text-sm mb-1">All caught up!</h3>
                          <p className="text-gray-500 text-xs">You've read all your notifications</p>
                        </div>
                      ) : (
